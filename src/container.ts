...
export function removeContainer(id: any){
    console.log(id);
...
export async function createAndRunContainer(port: number,  playground_id: string){
    ...
    const {stdout , error}: { stdout: string, error: Error | null } =  await new Promise( (resolve , reject) => {
        ...
        if(JSON.parse(stdout).message){
            ...
            let stdout2;
            const {stdout , error}: { stdout: string, error: Error | null } = await new Promise((resolve , reject) => {exec(startCommand ,
                (error, stdout, stderr) => {
